...
| 10 | Claims | P2-Financial | MODIFY | 3.5/10 | 7.0/10 | **+3.5** | Endpoint count 100% accurate (39=39, 3rd perfect match). Model names 100% correct (8=8). Most accurate hub of all 14 services (~87%). 56+ real tests (hub: "likely empty"). Reports controller missing RolesGuard. Filing window (9-month) + insurance routing ($10K) NOT implemented. ClaimContact dead model (no API). | `PST-10-claims.md` |
| 15 | Contracts | P2-Financial | MODIFY | 3.5/10 | 7.5/10 | **+4.0** | Endpoint count perfect (58=58) but worst path accuracy (~65%). Hub `/fuel-surcharges` vs code `/fuel-tables` (0% match). 9 spec files / 60 tests (hub: "2 files, boilerplate"). 6/8 controllers missing RolesGuard. FuelSurchargeTier missing tenantId. ContractClause undocumented. Status machine misses 3 states. Frontend "Not Built" confirmed. | `PST-15-contracts.md` |
| 16 | Agents | P2-Financial | MODIFY | 2.0/10 | 6.5/10 | **+4.5** | Most accurate hub (~92%). Endpoint count 100% (43=43). Paths ~100%. Model names 100% (9=9). 3/6 controllers missing RolesGuard. Tenant leak in rankings. 28 tests (hub: "None"). Lead conversion works. AgentTier missing PLATINUM. No commission calc engine. | `PST-16-agents.md` |
| 17 | Credit | P2-Financial | MODIFY | 2.0/10 | 7.0/10 | **+5.0** | Endpoint count 100% (31=31). Paths ~100%. Model names 100% (5/5). 0/5 controllers use RolesGuard in @UseGuards. Collections soft-delete gap. No hold→limit auto-suspend. 58 tests / 768 LOC. Module exports all 5 services. Frontend "Not Built" confirmed. | `PST-17-credit.md` |
| 18 | Factoring Internal | P2-Financial | MODIFY | 2.0/10 | 7.5/10 | **+5.5** | Best endpoint docs of all 18 services (~100% paths + count, 30=30). **P0 SECURITY: apiKey plaintext** in GET responses. 3/5 controllers missing RolesGuard. companyCode cross-tenant bug. 49 tests (hub: "None"). 10 EventEmitter events undocumented. Lazy NOA auto-expiration exists. **BATCH 3 COMPLETE.** | `PST-18-factoring.md` |
| 19 | Analytics | P2-Platform | MODIFY | 2.0/10 | 7.8/10 | **+5.8** | **NEW LARGEST SCORE DELTA (+5.8). BATCH 4 START.** Security "Unknown" FALSE — 100% guards on all 6 controllers (first P2 with 0 gaps). 41 endpoints (hub: 40). 11 Prisma models (hub: 9-10, missing LaneAnalytics). 42 tests / 777 LOC. .bak 52K LOC safe to delete (97% reduction). DataQueryService has 3 stub endpoints (mock data). Frontend "Not Built" confirmed. ~100% path accuracy. All 7 enums match. No raw SQL. | `PST-19-analytics.md` |
| 20 | Workflow | P2-Platform | MODIFY | 2.0/10 | 7.5/10 | **+5.5** | Endpoint count 100% (35=35, 8th perfect match). ~100% path accuracy. 7 Prisma models (hub: 6, missing StepExecution). **Execution engine is pass-through stub** — action library metadata-only, 5/6 step types unimplemented. 3/5 controllers missing RolesGuard (@Roles decorative). No scheduled workflow runner. ApprovalRequest soft-delete gap. 4 spec files / ~38 tests / 626 LOC (hub: "3 files"). .bak 2,301 LOC safe to delete. 2,950 active LOC. | `PST-20-workflow.md` |
| 21 | Integration Hub | P2-Platform | MODIFY | 2.0/10 | 8.0/10 | **+6.0** | **LARGEST SCORE DELTA (+6.0).** Endpoint count 100% (45=45, 9th perfect match). Security 100% guards (hub: "Unknown" — FALSE). AES-256-GCM encryption VERIFIED + tested. Hub DataTransformation model is PHANTOM (actual: TransformationTemplate). 9 Prisma models (hub: 7). 46 tests / 826 LOC (hub: "None"). P1: EncryptionService hardcoded fallback key. P2: testConnection() uses Math.random(). .bak 2,682 LOC safe to delete. | `PST-21-integration-hub.md` |
| 22 | Search | P2-Platform | MODIFY | 2.0/10 | 7.5/10 | **+5.5** | Endpoint count 100% (27=27, 10th perfect match). Paths ~100%. **P0 CRITICAL: Elasticsearch queries have ZERO tenantId filtering** — cross-tenant data leakage. Hub claims "mandatory tenantId filter" — FALSE. 2/4 controllers missing RolesGuard. 6 Prisma models (hub: 1 — misses SearchHistory, SearchIndex, SearchIndexQueue, SearchSuggestion, SearchSynonym). 37 tests / 617 LOC. No auto-indexing (no event listeners, no cron). QueueProcessor handles only 5/7 entity types. deleteSynonym cross-tenant bug. **BATCH 4 COMPLETE.** | `PST-22-search.md` |
| 23 | HR | P3-Future | MODIFY | 2.0/10 | 7.0/10 | **+5.0** | Endpoint count 100% (35=35, 11th perfect match). Paths ~100%. Hub calls code "Scaffolded" — FALSE: production CRUD with PTO balance management, circular hierarchy detection, auto employee numbering, approval workflows. **3 HARD DELETE BUGS** (Department, Position, Location use `.delete()` instead of soft delete). **6/7 services skip `deletedAt: null` filter** (worst compliance rate). 10 Prisma models (hub: 6 — misses TimeOffBalance, EmploymentHistory, OnboardingChecklist, OnboardingTask). Hub data model ~48% field accuracy (wrong enum names, phantom fields). 48 tests / 7 spec files / 767 LOC (hub: "None" — 15th false claim). JwtAuthGuard 100%, RolesGuard 0% (decorative @Roles). 6 EventEmitter events undocumented. Module exports nothing. No .bak directory. **BATCH 5 START.** | `PST-23-hr.md` |
...
| ------- | ----- | -------- |
| AFFIRM | 0 | |
| MODIFY | 23 | Auth & Admin, Dashboard, CRM, Sales, TMS Core, Carriers, Accounting, Commission, Load Board, Customer Portal, Documents, Communication, Carrier Portal, Claims, Contracts, Agents, Credit, Factoring, Analytics, Workflow, Integration Hub, Search, HR |
| REVERSE | 0 | |
| DEFER | 0 | |
