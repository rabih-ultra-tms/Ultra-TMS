...
1. **Commission Trigger:** Commission is calculated when an invoice status changes to PAID. Calculated commission = (invoice total x commission rate) based on the sales rep's active commission plan. Commission is NOT calculated on drafts or unpaid invoices. **Wired:** Event listener `CommissionEventsListener` has `@OnEvent('invoice.paid')` which calls `calculateLoadCommission()` for all linked loads (via loadId or orderId).
