...
// Inland quote status labels and colors
const INLAND_STATUS_LABELS: Record<string, string> = {
  draft: 'Draft',
  sent: 'Sent',
  viewed: 'Viewed',
  accepted: 'Accepted',
...
const INLAND_STATUS_COLORS: Record<string, string> = {
  draft: 'text-gray-500 border-gray-300',
  sent: 'text-blue-500 border-blue-300',
  viewed: 'text-purple-500 border-purple-300',
  accepted: 'text-green-500 border-green-300',
  rejected: 'text-red-500 border-red-300',
  expired: 'text-orange-500 border-orange-300',
}

interface InlandVersionHistoryProps {
  quoteId: string
  onRevisionCreated?: () => void
}

export function InlandVersionHistory({ quoteId, onRevisionCreated }: InlandVersionHistoryProps) {
  ...
  const { data: versions, isLoading } = trpc.inland.getVersions.useQuery({
    quoteId,
  })

  const { data: comparison, isLoading: compareLoading } = trpc.inland.compareVersions.useQuery(
    { quoteId1: compareVersion1, quoteId2: compareVersion2 },
    { enabled: !!compareVersion1 && !!compareVersion2 && compareVersion1 !== compareVersion2 }
  )

  const createRevisionMutation = trpc.inland.createRevision.useMutation({
    onSuccess: (newQuote) => {
      toast.success(`Revision v${newQuote.version} created`)
      setCreateDialogOpen(false)
      setChangeNotes('')
      utils.inland.getVersions.invalidate({ quoteId })
      onRevisionCreated?.()
    },
    onError: (error) => {
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Link
                      href={`/inland/history/${version.id}`}
                    ...
                    <Badge
                      variant="outline"
                      className={INLAND_STATUS_COLORS[version.status] || ''}
                    >
                      {INLAND_STATUS_LABELS[version.status] || version.status}
                    </Badge>
                  </div>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will create a new version of the inland quote as a draft, preserving the
              original version.
            </DialogDescription>
          </DialogHeader>
