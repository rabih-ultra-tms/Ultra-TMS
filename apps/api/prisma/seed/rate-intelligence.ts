    ...
    for (let i = 0; i < 20; i++) {
      await prisma.rateQuery.create({
        data: {
          ...
          provider: faker.helpers.arrayElement(['DAT', 'TRUCKSTOP', 'INTERNAL']),
          lowRate: parseFloat(faker.commerce.price({ min: 800, max: 3500 })),
          highRate: parseFloat(faker.commerce.price({ min: 1200, max: 5000 })),
          avgRate: parseFloat(faker.commerce.price({ min: 1000, max: 4000 })),
          confidence: faker.helpers.arrayElement(['HIGH', 'MEDIUM', 'LOW']),
          loadVolume: faker.number.int({ min: 10, max: 500 }),
          truckVolume: faker.number.int({ min: 10, max: 500 }),
          marketTrend: faker.helpers.arrayElement(['UP', 'DOWN', 'FLAT']),
          queryHash: `${Date.now()}-${i}-${faker.string.alphanumeric(16)}`,
          cachedUntil: new Date(Date.now() + 1000 * 60 * 60),
