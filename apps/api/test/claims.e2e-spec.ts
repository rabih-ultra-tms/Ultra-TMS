...
describe('Claims API E2E', () => {
  ...
  it('creates, progresses, resolves, and reports on a claim', async () => {
    ...
    const fileRes = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(fileRes.body.status).toBe('SUBMITTED');
    ...
    const assignRes = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(assignRes.body.assignedToId).toBe('adjuster-1');
    ...
    const noteRes = await request(app.getHttpServer())
      ...
      .expect(201);
    expect(noteRes.body.note).toBe('Investigation started');
    ...
    expect(Number(approveRes.body.approvedAmount)).toBe(400);
    ...
    const payRes = await request(app.getHttpServer())
      ...
      .send({ amount: 400 })
      .expect(200);
    expect(payRes.body.status).toBe('CLOSED');
    expect(Number(payRes.body.paidAmount)).toBe(400);
