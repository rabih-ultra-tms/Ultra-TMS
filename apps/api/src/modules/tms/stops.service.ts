  ...
  async create(tenantId: string, orderId: string, userId: string, dto: CreateStopDto) {
    ...
    const stop = await this.prisma.stop.create({
      data: {
        ...
        appointmentTimeStart: dto.appointmentTime,
        createdById: userId,
        updatedById: userId,
      },
    });
  ...
  async update(tenantId: string, userId: string, id: string, dto: UpdateStopDto) {
    ...
    const updated = await this.prisma.stop.update({
      ...
      data: {
        ...
        facilityName: dto.companyName ?? stop.facilityName,
        addressLine1: dto.address ?? stop.addressLine1,
        city: dto.city ?? stop.city,
        state: dto.state ?? stop.state,
        postalCode: dto.zip ?? stop.postalCode,
        contactName: dto.contactName ?? stop.contactName,
        contactPhone: dto.phone ?? stop.contactPhone,
        contactEmail: dto.email ?? stop.contactEmail,
        appointmentDate: dto.appointmentDate ? new Date(dto.appointmentDate) : stop.appointmentDate,
        appointmentTimeStart: dto.appointmentTime ?? stop.appointmentTimeStart,
        specialInstructions: dto.instructions ?? stop.specialInstructions,
