...
export function CarrierForm({
  ...
  return (
    <FormPage<CarrierFormValues>
      ...
      description={
        isEdit
          ? 'Update carrier details and settings.'
    ...
    >
      {(form: import('react-hook-form').UseFormReturn<CarrierFormValues>) => (
        <div className="grid gap-6">
          ...
          <FmcsaLookup
            onAutoFill={(data) => {
              ...
              if (data.mcNumber)
                form.setValue('mcNumber', data.mcNumber, { shouldDirty: true });
              if (data.dotNumber)
                form.setValue('dotNumber', data.dotNumber, {
                  shouldDirty: true,
                });
              if (data.address)
              ...
              if (data.zip)
                form.setValue('zip', data.zip, { shouldDirty: true });
              if (data.phone)
          ...
          <FormSection
            ...
            description="Payment terms and billing contact."
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField
                control={form.control}
          ...
          <FormSection
            title="Factoring"
            description="Factoring company details if applicable."
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField
                control={form.control}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        type="email"
                        placeholder="payments@factoring.com"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
          ...
          <FormSection
            title="Insurance"
            description="Carrier insurance details and limits."
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField
                control={form.control}
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        {...field}
                        value={
                          field.value !== undefined ? field.value / 100 : ''
                        }
                        onChange={(e) => {
          ...
          <FormSection
            ...
            description="Equipment types and fleet size."
          >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {EQUIPMENT_TYPES.map((type) => (
                    <div key={type} className="flex items-center space-x-2">
                      <Checkbox
                        id={`eq-${type}`}
                        checked={(form.watch('equipmentTypes') ?? []).includes(
                          type
                        )}
                        onCheckedChange={(checked) => {
                          const current =
                            form.getValues('equipmentTypes') ?? [];
                          form.setValue(
                            'equipmentTypes',
                            checked
                              ? [...current, type]
