...
export function CarrierDocumentsManager({
  carrierId,
}: CarrierDocumentsManagerProps) {
  const { data: rawDocs, isLoading } = useCarrierDocuments(carrierId);
  const documents: OperationsCarrierDocument[] = Array.isArray(rawDocs)
    ? rawDocs
    : [];

  const createDocument = useCreateCarrierDocument(carrierId);
  const deleteDocument = useDeleteCarrierDocument(carrierId);
  const getDownloadUrl = useCarrierDocumentDownloadUrl(carrierId);
  ...
  const set =
    (field: keyof Omit<DocumentFormData, 'file'>) => (value: string) =>
  ...
  const handleDownload = async (doc: OperationsCarrierDocument) => {
    try {
      const result = await getDownloadUrl.mutateAsync(doc.id);
      const url =
        (result as { downloadUrl?: string })?.downloadUrl ??
        (result as { data?: { downloadUrl?: string } })?.data?.downloadUrl;
      if (url) {
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between space-y-0">
          <div>
            ...
            <CardDescription>
              Carrier documents and compliance records.
            </CardDescription>
          </div>
          <Button
            size="sm"
                    ...
                    <tr key={doc.id} className="border-b last:border-0">
                      ...
                      <td className="py-2 pr-4 text-muted-foreground">
                        {(doc.expirationDate ?? doc.expiryDate)
                          ? String(doc.expirationDate ?? doc.expiryDate).slice(
                              0,
                              10
                            )
                          : '—'}
                      ...
                      <td className="py-2 pr-4">
                        <Badge
                          variant={STATUS_COLORS[doc.status] ?? 'secondary'}
                        >
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                PDF, Word, Excel, images, or text files (max 25MB).
              </p>
            </div>
          </div>
          <DialogFooter>
