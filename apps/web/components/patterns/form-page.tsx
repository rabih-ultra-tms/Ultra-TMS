...
interface FormPageProps<T extends FieldValues> {
  ...
  className?: string;
  /** Optional extra sections rendered after form content (outside the form element). */
  extraSections?: (form: UseFormReturn<T>) => React.ReactNode;
}
...
export function FormPage<T extends FieldValues>({
  ...
  return (
    <div
      className={cn(
        'flex flex-col min-h-screen bg-background pb-20',
        className
      )}
    >
      ...
      <div className="flex-1 p-6 md:p-8 max-w-5xl mx-auto w-full">
        ...
        {extraSections && extraSections(form)}
      </div>
