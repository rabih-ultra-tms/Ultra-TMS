...
function getCustomerEmail(load: Load): string {
  return (
    load.order?.customer?.contactEmail || load.order?.customer?.email || ''
  );
}
...
function getAvailableEmails(load: Load): EmailType[] {
  ...
  if (
    [LoadStatus.TENDERED, LoadStatus.ACCEPTED, LoadStatus.DISPATCHED].includes(
      s
    )
  ) {
...
export function LoadDetailHeader({ load }: LoadDetailHeaderProps) {
  ...
  const [activeEmailType, setActiveEmailType] =
    useState<EmailType>('rate_confirmation');
  ...
  const {
    generate: generateBol,
    download: downloadBol,
    isGenerating: isBolGenerating,
    hasGenerated: hasBol,
  } = useBol(load.id);
  ...
  const handleReject = () => {
    rejectMutation.mutate(rejectReason ? { reason: rejectReason } : undefined, {
      onSuccess: () => {
  ...
  const attachments =
    activeEmailType === 'rate_confirmation'
      ? [
          {
            name: `Rate-Con-${load.loadNumber}.pdf`,
            url: `/loads/${load.id}/rate-confirmation`,
  ...
  return (
    <div className="flex flex-col gap-4 border-b bg-background px-6 py-4 shadow-sm">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <Link href="/operations/loads" className="hover:text-foreground">
          Operations
        </Link>
        <span>/</span>
        <Link href="/operations/loads" className="hover:text-foreground">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          {load.carrier ? (
            ...
            <span className="text-red-500 font-medium bg-red-50 px-2 py-0.5 rounded text-sm">
              Unassigned
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push(`/operations/loads/${load.id}/edit`)}
          >
      ...
      <Dialog open={tenderDialogOpen} onOpenChange={setTenderDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                step="0.01"
                placeholder={
                  load.carrierRate ? String(load.carrierRate) : 'Enter rate'
                }
                value={tenderRate}
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setTenderDialogOpen(false)}
            >
              Cancel
            </Button>
            <Button
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Rejecting the tender will set the load back to Pending and
