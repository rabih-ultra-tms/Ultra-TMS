  ...
  return (
    ...
    >
      ...
      <CardContent className="space-y-3">
            ...
            {permits.statePermits.length === 0 ? (
              ...
              permits.statePermits.map((sp) => (
                <div key={sp.stateCode} className="flex justify-between">
                  <span className="font-medium">{sp.state ?? sp.stateName}</span>
                  <span>
                    {(sp.reasons ?? []).slice(0, 2).join(', ')} — {formatCurrency(sp.estimatedFee ?? sp.totalCost / 100)}
                  </span>
                </div>
              ))
            )}
        ...
        {/* Warnings */}
        {(permits.warnings ?? permits.statePermits?.flatMap(p => p.warnings ?? []) ?? []).length > 0 && (
          <div className="space-y-1">
            {(permits.warnings ?? permits.statePermits?.flatMap(p => p.warnings ?? []) ?? []).slice(0, 2).map((w, i) => (
