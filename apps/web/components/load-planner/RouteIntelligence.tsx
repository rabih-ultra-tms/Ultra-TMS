...
import type { StateSegment, RouteResult } from '@/lib/load-planner/route-calculator'
        ...
        {expandedSections.has('summary') && result && (
          <div className="pl-6 space-y-3">
            <div className="grid grid-cols-3 gap-4 text-center">
              ...
              <div>
                <div className="text-xl font-bold">{result.estimatedDriveTime ?? 'N/A'}</div>
              ...
              <div className="pl-6 space-y-3">
                <div className="grid grid-cols-2 gap-2 text-sm pb-2 border-b">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">${state.permitSummary.totalPermitFees.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Escort Costs:</span>
                    <span className="font-medium">${state.permitSummary.totalEscortCost.toLocaleString()}</span>
                  </div>
                </div>
        ...
        {state.seasonalWarnings?.hasRestrictions && (
          <>
            <SectionHeader
              ...
              badge={<Badge variant="destructive">{state.seasonalWarnings.affectedStates.length} States</Badge>}
            />
                ...
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-blue-700 mt-1">Weight limits reduced on secondary roads in:</div>
        ...
        {state.bridgeWarnings?.hasIssues && (
          <>
            <SectionHeader
              ...
              badge={<Badge variant="destructive">{state.bridgeWarnings.bridges.length} Issues</Badge>}
            />
            {expandedSections.has('bridges') && (
              <div className="pl-6 space-y-2">
                {state.bridgeWarnings.bridges.slice(0, 5).map(({ bridge, clearanceResult }) => (
                  <BridgeWarningCard key={bridge.id} bridge={bridge} clearanceResult={clearanceResult} />
                ))}
...
function StateSegmentBadge({ segment }: { segment: StateSegment }) {
  return (
...
function BridgeWarningCard({ bridge, clearanceResult }: {
  bridge: LowClearanceBridge
  clearanceResult: { clears: boolean; clearance: number; deficit: number; severity: 'ok' | 'caution' | 'warning' | 'danger' }
}) {
...
function PermitBreakdownCard({ permit, isExpanded, onToggle }: {
  permit: DetailedPermitRequirement; isExpanded: boolean; onToggle: () => void
}) {
  const hasPermitRequired = permit.oversizeRequired || permit.overweightRequired

  const getRowColors = () => {
    if (permit.isSuperload) return { bg: 'bg-red-50', hover: 'hover:bg-red-100', border: 'border-red-200' }
    if (permit.overweightRequired) return { bg: 'bg-orange-50', hover: 'hover:bg-orange-100', border: 'border-orange-200' }
    if (permit.oversizeRequired) return { bg: 'bg-amber-50', hover: 'hover:bg-amber-100', border: 'border-amber-200' }
    return { bg: 'bg-green-50', hover: 'hover:bg-green-100', border: 'border-green-200' }
  }
    ...
    <div className={`border rounded-lg overflow-hidden ${colors.border}`}>
      ...
      >
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-medium">{permit.state ?? permit.stateName}</span>
          <Badge variant="outline" className="text-xs font-mono">{permit.stateCode}</Badge>
          <span className="text-sm text-muted-foreground">
            {(permit.distanceInState ?? 0) > 0 ? `${(permit.distanceInState ?? 0).toFixed(0)} mi` : ''}
          </span>
          {permit.isSuperload && <Badge className="text-xs bg-red-100 text-red-700 border-red-200">Superload</Badge>}
          {permit.overweightRequired && !permit.isSuperload && <Badge className="text-xs bg-orange-100 text-orange-700 border-orange-200">Overweight</Badge>}
          {permit.oversizeRequired && !permit.overweightRequired && !permit.isSuperload && <Badge className="text-xs bg-amber-100 text-amber-700 border-amber-200">Oversize</Badge>}
          {!hasPermitRequired && <Badge className="text-xs bg-green-100 text-green-700 border-green-200">Legal</Badge>}
          <div className="flex items-center gap-1 ml-1">
            {hasNightRestriction && <span title="No night travel"><Moon className="h-3.5 w-3.5 text-indigo-500" /></span>}
            {hasWeekendRestriction && <span title="No weekend travel"><Calendar className="h-3.5 w-3.5 text-purple-500" /></span>}
            {hasSeasonalRestriction && <span title="Seasonal restrictions"><Snowflake className="h-3.5 w-3.5 text-blue-500" /></span>}
          </div>
          {(permit.escortsRequired > 0 || permit.poleCarRequired || permit.policeEscortRequired) && (
            <div className="flex items-center gap-0.5 ml-1">
              ...
              {permit.poleCarRequired && <span title="Pole car required"><Ruler className="h-3.5 w-3.5 text-blue-600" /></span>}
              {permit.policeEscortRequired && <span title="Police escort required"><Siren className="h-3.5 w-3.5 text-red-600" /></span>}
            </div>
          )}
        </div>
        <div className="flex items-center gap-2">
          {hasPermitRequired && <span className="font-bold text-slate-700">${permit.estimatedFee ?? 0}</span>}
        ...
        <div className="p-4 space-y-4 border-t bg-white">
          ...
          {(permit.calculationDetails ?? []).length > 0 && (
            <div>
              ...
              <ul className="text-sm space-y-1">
                ...
                <li className="font-bold border-t pt-1 mt-2">Total: ${permit.estimatedFee ?? 0}</li>
              </ul>
            </div>
          )}
          {permit.escortsRequired > 0 && (
          ...
          {permit.source && (
            <div className="text-xs text-slate-500 border-t pt-3">
              ...
              <a href={permit.source.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline inline-flex items-center gap-1">
...
function EscortCostBreakdownCard({ breakdown }: { breakdown: EscortCostBreakdown }) {
  const [isExpanded, setIsExpanded] = useState(false)
  if (breakdown.escortCount === 0 && !breakdown.needsPoleCar && !breakdown.needsPoliceEscort) return null

  return (
    <div className="border rounded-lg overflow-hidden bg-blue-50/50">
      <button onClick={() => setIsExpanded(!isExpanded)} className="w-full flex justify-between items-center p-3 bg-blue-50 hover:bg-blue-100 text-left">
        <div className="flex items-center gap-2">
        ...
        <div className="p-4 space-y-4 border-t bg-white">
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="p-2 bg-slate-50 rounded"><div className="text-muted-foreground">Escort/Pilot Car</div><div className="font-medium">${breakdown.rates.escortPerDay}/day</div></div>
              <div className="p-2 bg-slate-50 rounded"><div className="text-muted-foreground">Height Pole Car</div><div className="font-medium">${breakdown.rates.poleCarPerDay}/day</div></div>
              <div className="p-2 bg-slate-50 rounded"><div className="text-muted-foreground">Police Escort</div><div className="font-medium">${breakdown.rates.policePerHour}/hour</div></div>
            </div>
          </div>
          <div>
            <h4 className="text-sm font-medium text-slate-700 mb-2">Trip Estimate</h4>
            <div className="text-sm text-slate-600">
              <span className="font-medium">{breakdown.tripDays} day{breakdown.tripDays > 1 ? 's' : ''}</span> ({breakdown.tripHours} driving hours)
            </div>
          </div>
          <div>
            ...
            <div className="space-y-2 text-sm">
              {breakdown.escortCount > 0 && (
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded">
                  <span className="text-slate-600">{breakdown.escortCount} Escort{breakdown.escortCount > 1 ? 's' : ''} x ${breakdown.rates.escortPerDay}/day x {breakdown.tripDays} days</span>
                  <span className="font-medium text-slate-900">${breakdown.totalEscortCost.toLocaleString()}</span>
                </div>
              )}
              {breakdown.needsPoleCar && (
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded">
                  <span className="text-slate-600">Height Pole Car x ${breakdown.rates.poleCarPerDay}/day x {breakdown.tripDays} days</span>
                  <span className="font-medium text-slate-900">${breakdown.totalPoleCarCost.toLocaleString()}</span>
                </div>
              )}
              {breakdown.needsPoliceEscort && (
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded">
                  <span className="text-slate-600">Police Escort x ${breakdown.rates.policePerHour}/hr x {breakdown.tripHours} hours</span>
                  <span className="font-medium text-slate-900">${breakdown.totalPoliceCost.toLocaleString()}</span>
                </div>
              )}
          ...
          {breakdown.perState.length > 0 && breakdown.perState.some(s => s.stateCost > 0) && (
            <div>
              ...
              <div className="space-y-1">
                {breakdown.perState.filter(s => s.stateCost > 0).map(st => (
                  <div key={st.stateCode} className="flex justify-between text-xs p-2 bg-slate-50 rounded">
                    <span className="text-slate-600">
                      {st.stateName} ({st.distanceMiles.toFixed(0)} mi, ~{st.daysInState} day{st.daysInState !== 1 ? 's' : ''})
                      {st.escortCountInState > 0 && <span className="text-blue-600 ml-1">[{st.escortCountInState} escort{st.escortCountInState > 1 ? 's' : ''}]</span>}
                      {st.poleCarRequiredInState && <span className="text-amber-600 ml-1">[pole car]</span>}
                      {st.policeRequiredInState && <span className="text-red-600 ml-1">[police]</span>}
                    </span>
                    <span className="font-medium">${st.stateCost.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
...
function TruckPermitStatusCard({ truck }: { truck: TruckCargoSpecs }) {
  const needsPermit = truck.isOversize || truck.isOverweight
  const statusColor = needsPermit ? 'bg-amber-50 border-amber-200' : 'bg-green-50 border-green-200'
  const statusIcon = needsPermit ? <AlertTriangle className="h-4 w-4 text-amber-500" /> : <CheckCircle2 className="h-4 w-4 text-green-500" />
  const LEGAL_LIMITS = { width: 8.5, height: 13.5, length: 53, weight: 80000 }
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">Load {truck.truckIndex + 1}</div>
          </div>
        </div>
        <Badge variant={needsPermit ? 'destructive' : 'secondary'} className="text-xs">{needsPermit ? 'Permit Required' : 'Legal'}</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2 mt-3 text-xs">
        <div><div className="text-muted-foreground">Width</div><div className={`font-medium ${truck.width > LEGAL_LIMITS.width ? 'text-amber-600' : ''}`}>{truck.width.toFixed(1)}&apos;{truck.width > LEGAL_LIMITS.width && <span className="text-amber-500"> !</span>}</div></div>
        <div><div className="text-muted-foreground">Height</div><div className={`font-medium ${truck.height > LEGAL_LIMITS.height ? 'text-amber-600' : ''}`}>{truck.height.toFixed(1)}&apos;{truck.height > LEGAL_LIMITS.height && <span className="text-amber-500"> !</span>}</div></div>
        <div><div className="text-muted-foreground">Length</div><div className={`font-medium ${truck.length > LEGAL_LIMITS.length ? 'text-amber-600' : ''}`}>{truck.length.toFixed(1)}&apos;{truck.length > LEGAL_LIMITS.length && <span className="text-amber-500"> !</span>}</div></div>
        <div><div className="text-muted-foreground">Weight</div><div className={`font-medium ${truck.grossWeight > LEGAL_LIMITS.weight ? 'text-amber-600' : ''}`}>{(truck.grossWeight / 1000).toFixed(0)}k{truck.grossWeight > LEGAL_LIMITS.weight && <span className="text-amber-500"> !</span>}</div></div>
      </div>
      {needsPermit && (
        <div className="flex gap-2 mt-2 flex-wrap">
          {truck.isOversize && <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded">Oversize</span>}
          {truck.isOverweight && <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded">Overweight</span>}
        </div>
      )}
    </div>
  )
...
function TruckRouteCard({ recommendation }: { recommendation: TruckRouteRecommendation }) {
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">{recommendation.cargoDescription}</div>
          </div>
        </div>
        <div className="flex gap-1">
          {recommendation.isOversize && <Badge variant="outline" className="text-xs bg-amber-100 text-amber-700 border-amber-300">Oversize</Badge>}
          {recommendation.isOverweight && <Badge variant="outline" className="text-xs bg-amber-100 text-amber-700 border-amber-300">Overweight</Badge>}
          {!recommendation.isOversize && !recommendation.isOverweight && <Badge variant="outline" className="text-xs bg-green-100 text-green-700 border-green-300">Legal</Badge>}
        </div>
      </div>
      <div className="mt-2 flex items-center gap-2 text-sm">
        <ArrowRight className="h-4 w-4 text-violet-500" />
        <span className="font-medium text-violet-700">{recommendation.recommendedRouteName}</span>
        {needsSpecialRoute && <Badge variant="secondary" className="text-xs bg-blue-100 text-blue-700">Different Route</Badge>}
      </div>
