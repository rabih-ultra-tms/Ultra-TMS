...
// Self-contained types for state permit data
interface DimensionSurcharge { threshold: number; fee: number }
interface WeightBracket { upTo: number; fee: number }
...
export interface StatePermitData {
  ...
  legalLimits: {
    maxWidth: number
    maxHeight: number
    maxLength: { single: number; combination: number }
    maxWeight: { gross: number; perAxle?: { single: number; tandem: number; tridem?: number } }
  }
  oversizePermits: {
    singleTrip: {
      baseFee: number
      ...
      surchargeModel?: 'cumulative' | 'tiered'
      processingTime: string
      validity: string
    }
    annual?: { baseFee: number; maxWidth?: number; maxHeight?: number; maxLength?: number; maxWeight?: number; restrictions?: string[] }
  }
  overweightPermits: {
    singleTrip: {
      baseFee: number
      ...
      extraLegalFees?: { perTrip?: number }
    }
  }
  escortRules: {
    width: { oneEscort: number; twoEscorts?: number; front?: boolean; rear?: boolean }
    height?: { poleCar: number }
    length?: { oneEscort: number; twoEscorts?: number }
    policeEscort?: { width?: number; height?: number; fee: number }
  }
  travelRestrictions: {
  ...
  contact: { agency: string; phone: string; email?: string; website: string; permitPortal?: string }
  superloadThresholds?: { width?: number; height?: number; length?: number; weight?: number; requiresRouteSurvey?: boolean; requiresBridgeAnalysis?: boolean }
  bridgeAnalysis?: { weightThreshold: number; widthThreshold?: number; estimatedCostMin: number; estimatedCostMax: number; processingTime: string; notes?: string[] }
  kpraLimit?: { maxDistance: number; enforced: boolean; notes: string }
  specialJurisdictions?: Array<{ code: string; name: string; agency: string; phone: string; website: string; geoBounds: { minLat: number; maxLat: number; minLng: number; maxLng: number }; additionalPermitRequired: boolean; additionalFees: { oversizeBase: number; overweightBase: number }; restrictions?: string[]; notes?: string[] }>
  restrictedRoutes?: Array<{ route: string; maxWidth: number; maxHeight: number; maxWeight?: number; permitsAvailable: boolean; note: string }>
  notes?: string[]
  lastVerified?: string
}
