...
function scoreRoute(
  ...
  const maxCost = Math.max(...allRoutes.map(r => r.estimatedCosts?.total ?? 0), 1)
  const costScore = (1 - (route.estimatedCosts?.total ?? 0) / maxCost) * 100
...
function generateReasoning(
  ...
  const sortedByCost = [...allRoutes].sort((a, b) => (a.estimatedCosts?.total ?? 0) - (b.estimatedCosts?.total ?? 0))
  if (sortedByCost[0]?.id === route.id) {
    if (sortedByCost.length > 1) {
      const diff = (sortedByCost[1]?.estimatedCosts?.total ?? 0) - (route.estimatedCosts?.total ?? 0)
  ...
  const sortedByPermits = [...allRoutes].sort((a, b) => (a.estimatedCosts?.permits ?? 0) - (b.estimatedCosts?.permits ?? 0))
  if (sortedByPermits[0]?.id === route.id && sortedByPermits.length > 1) {
    const diff = (sortedByPermits[1]?.estimatedCosts?.permits ?? 0) - (route.estimatedCosts?.permits ?? 0)
    if (diff > 0) {
  ...
  const sortedByEscorts = [...allRoutes].sort((a, b) => (a.estimatedCosts?.escorts ?? 0) - (b.estimatedCosts?.escorts ?? 0))
  if (sortedByEscorts[0]?.id === route.id && sortedByEscorts.length > 1) {
    const diff = (sortedByEscorts[1]?.estimatedCosts?.escorts ?? 0) - (route.estimatedCosts?.escorts ?? 0)
    if (diff > 0) {
  ...
  if (sortedByStates[0]?.id === route.id && sortedByStates.length > 1) {
    if ((sortedByStates[1]?.statesTraversed.length ?? 0) > route.statesTraversed.length) {
...
function generateAlternativeConsiderations(
  ...
  // Cost comparison
  if ((route.estimatedCosts?.total ?? 0) > (recommended.estimatedCosts?.total ?? 0)) {
    const diff = (route.estimatedCosts?.total ?? 0) - (recommended.estimatedCosts?.total ?? 0)
...
export function analyzeRouteRecommendation(
  ...
  if (nextBest) {
    const savings = (nextBest.estimatedCosts?.total ?? 0) - (recommended.estimatedCosts?.total ?? 0)
    if (savings > 0) {
      costSavings = {
        amount: savings,
    ...
    if (truck.isOversize || truck.isOverweight) {
      ...
      if (routesWithoutSuperload.length > 0) {
        ...
        const bestNoSuperload = routesWithoutSuperload
          .sort((a, b) => (a.estimatedCosts?.total ?? 0) - (b.estimatedCosts?.total ?? 0))[0]
      ...
      if (truck.isOversize && truck.isOverweight) {
        ...
        reasoning.push(`Oversize load (${truck.cargo.width.toFixed(1)}' W x ${truck.cargo.height.toFixed(1)}' H)`)
      } else {
