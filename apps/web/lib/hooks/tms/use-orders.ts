...
function unwrapPaginated<T>(response: unknown): {
  data: T[];
...
export function useBulkUpdateOrderStatus() {
  ...
  return useMutation({
    ...
    onSuccess: ({ updated, failed }) => {
      queryClient.invalidateQueries({ queryKey: orderKeys.lists() });
      if (failed > 0) {
        toast.warning(`Updated ${updated} orders, ${failed} failed`);
      } else {
...
function mapFormToApi(formData: OrderFormValues, status: 'PENDING' | 'BOOKED') {
  return {
    ...
    temperatureMin:
      formData.equipmentType === 'REEFER' ? toNum(formData.tempMin) : undefined,
    temperatureMax:
      formData.equipmentType === 'REEFER' ? toNum(formData.tempMax) : undefined,
    ...
    stops: (formData.stops ?? []).map(
      (stop: OrderFormValues['stops'][number], index: number) => ({
        stopType: stop.type,
        stopSequence: index,
